...
export default function ExperienceCard({
  ...
  return (
    <div className="mb-12 md:grid md:grid-cols-2 md:gap-8 items-center group">
      {/* LEFT SIDE - Duration (shown when card is on the right) */}
      {isLeft ? (
      ...
      {/* CARD — same layout every time, only the grid column changes */}
      {isLeft ? (
      ...
      {/* RIGHT SIDE - Duration (shown when card is on the left) */}
      {!isLeft ? (
