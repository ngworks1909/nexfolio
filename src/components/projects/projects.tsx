...
export default function Projects() {
  ...
  const visibleProjects = showArchives ? projects : projects.slice(0, 3);
  ...
  return (
    <section id="projects" className="bg-background-dark font-display min-h-screen flex flex-col antialiased selection:bg-primary selection:text-background-dark">
      <div className="relative flex h-full grow flex-col overflow-x-hidden">
        <Reveal as="header" className="w-full px-6 pt-16 pb-8 md:px-12 md:pt-24 md:pb-12 lg:px-20 lg:pt-32 lg:pb-16 flex flex-col items-center text-center">
        ...
        <section className="w-full px-6 pb-24 md:px-12 lg:px-20 flex justify-center">
          <div
            id="projects-grid"
            className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-350"
          >
            {visibleProjects.map((project, index) => (
              <Reveal
                ...
                as="article"
                delay={(index % 3) * 100}
                className="glass-card rounded-2xl overflow-hidden flex flex-col hover:-translate-y-2 group"
              >
        ...
        <div className="w-full flex justify-center pb-20">
          <button
            ...
            className={`group flex items-center gap-2 px-8 py-3 rounded-xl border font-bold transition-all duration-300 ${
              showArchives
                ? "border-primary/50 bg-primary/10 text-primary shadow-[0_0_20px_-5px_rgba(37,192,244,0.4)] hover:bg-primary/15 hover:shadow-[0_0_24px_-4px_rgba(37,192,244,0.5)]"
                : "border-white/10 bg-white/[0.02] text-gray-300 hover:border-primary/50 hover:bg-primary/10 hover:text-primary hover:shadow-[0_0_20px_-5px_rgba(37,192,244,0.4)]"
            }`}
          >
            <span>
              {showArchives
                ? "Hide Archives"
                : `View Archives (${projects.length - 3})`}
            </span>
            <span
              className={`material-symbols-outlined text-xl transition-transform duration-300 ${
                showArchives ? "rotate-180" : "group-hover:translate-y-0.5"
              }`}
              aria-hidden="true"
            >
              expand_more
